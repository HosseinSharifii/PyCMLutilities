...
def multi_panel_from_flat_data(
        data_file_string = 'c:/temp/test_data.xlsx',
        template_file_string=[],
        output_image_file_string = 'c:/temp/fig_2.png',
        dpi = 300):
    ...
    # Read in the data
    pandas_data = pd.read_excel(data_file_string, sheet_name='Data')
    ...
    for i,p_data in enumerate(panel_data):
        ...
        for j,y_d in enumerate(p_data['y_info']['series']):
            ...
            if (y_d['style'] == 'line'):
                ax.plot(x, y, label= y_d['field_label'],
                        linewidth = formatting['data_linewidth'])
            if (y_d['style'] == 'envolope'):
        ...
        ax.set_xlim(xlim)
        ax.set_xticks(x_display['global_x_ticks'])
        ...
        if (len(p_data['y_info']['series'])>1):
            ax.legend(loc=formatting['legend_location'],
